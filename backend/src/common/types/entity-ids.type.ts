import { Opaque } from './opaque.type';

export type UserID = Opaque<string, 'UsersID'>;
export type QuantityFurnitureInOrderEntityID = Opaque<
  string,
  'QuantityFurnitureInOrderEntityID'
>;
export type FurnitureID = Opaque<string, 'FurnitureID'>;
export type DeliveryID = Opaque<string, 'DeliveryID'>;
export type FavouriteFurnitureID = Opaque<string, 'FavouriteFurnitureID'>;
export type SizeID = Opaque<string, 'SizeID'>;
export type IsShowPriceID = Opaque<string, 'IsShowPriceID'>;
export type BrandID = Opaque<string, 'BrandID'>;
export type CategoryFurnitureID = Opaque<string, 'CategoryFurnitureID'>;
export type SubCategoryFurnitureID = Opaque<string, 'SubCategoryFurnitureID'>;
export type RefreshTokenID = Opaque<string, 'RefreshTokenID'>;
export type ColorID = Opaque<string, 'ColorID'>;
export type MaterialID = Opaque<string, 'MaterialID'>;
export type BannerID = Opaque<string, 'BannerID'>;
export type SiteStatisticID = Opaque<string, 'SiteStatisticID'>;
export type ShippingInfoID = Opaque<string, 'ShippingInfoID'>;
export type PaymentInfoID = Opaque<string, 'PaymentInfoID'>;
export type FurnitureStatisticID = Opaque<string, 'FurnitureStatisticID'>;
export type CalculateRateFurnitureID = Opaque<
  string,
  'CalculateRateFurnitureID'
>;
export type CustomerInfoID = Opaque<string, 'CustomerInfoID'>;
